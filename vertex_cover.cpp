...
class GradientDescent final : public VertexCoverSolver {
    ...
    VertexCover Solve(const Graph& graph, DebugInfo& debug_info) const {
        ...
        while (true) {
            ...
            for (auto elem : graph.AdjacentVertices(path[0])){
                if (checker(path, elem)) {
                    friends.push_back(std::make_pair(elem, 0));
                }
            }
            for (auto elem : graph.AdjacentVertices(path[path.size() - 1])){
                if (checker(path, elem)) {
                    friends.push_back(std::make_pair(elem, path.size() - 1));
                }
            }
...
class Metropolis final: public VertexCoverSolver {
    ...
    bool checker(std::vector<Graph::Vertex>& vertexset, Graph::Vertex& elem_to_check) const {
        bool flag = false;
        for (auto elem : vertexset) {
            if (elem == elem_to_check) {
                flag = true;
...
public:
    ...
    VertexCover Solve(const Graph& graph, DebugInfo& debug_info) const {
        ...
        while (true) {
            ...
            for (auto elem : graph.AdjacentVertices(*path.end())){
                friends.push_back(std::make_pair(elem, path.size() - 1));
            }
            */
